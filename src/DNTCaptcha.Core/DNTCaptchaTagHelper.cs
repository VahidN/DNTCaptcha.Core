...
public class DNTCaptchaTagHelper : DNTCaptchaTagHelperHtmlAttributes, ITagHelper
{
	...
	public void Process(TagHelperContext context, TagHelperOutput output)
	{
		...
		output.Content.AppendHtml(
			$"{string.Format(CultureInfo.InvariantCulture, CaptchaTemplate, 
				captchaImage.GetString(),
				ShowRefreshButton ? getRefreshButtonTagBuilder(ViewContext, captchaDivId, cookieToken).GetString() : "",
				textInput.GetString())}");
	...
	private TagBuilder getCaptchaImageTagBuilder(ViewContext viewContext, string encryptedText)
	{
		...
		var actionUrl = UseRelativeUrls
			? _urlHelper.Action(nameof(DNTCaptchaImageController.Show),
				nameof(DNTCaptchaImageController).Replace("Controller", string.Empty, StringComparison.Ordinal),
				new
				{
